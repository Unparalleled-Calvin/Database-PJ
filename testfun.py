...
def check_identity(cursor, id, passwd):
    sql = "select ID, passwd from person natural join card where ID = '{}' and passwd = '{}'".format(
        id, passwd)
    cursor.execute(sql)
    return cursor.rowcount
...
def select_dno_people(cursor, start, end):
    sql = "select cdno, count(*) as number from card where valid <> 0 and cdno is not null and carddate between '{}' and '{}' group by cdno".format(start, end)
    cursor.execute(sql)
    rows = cursor.fetchall()
    sql1 = "select dno from domitory except (select cdno from card where valid <> 0 and cdno is not null and carddate between '{}' and '{}')".format(
        start, end)
    cursor.execute(sql1)
    rows1 = cursor.fetchall()
    for row in rows1:
...
print('1\n')
print(select_dno_people(cur, '2021-05-30', '2021-05-30'))
